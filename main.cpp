...
vector<pair<int, int>> path(int x, int y, int steps, int sum,const int& total_steps ,const int& width, const int& height, vector<vector<int>> table, const int& ending_x, const int& ending_y) {
    ...
    //return empty so that the way doesn't continue to be checked
    return {};
}
...
void generate_advanced_table(int x, int y, int min_wall,int max_wall,int min_num,int max_num,int total_steps){
    ...
    for(int i = 0; i<= x-1;i++){
        for(int j=0; j <= y-1;j++){
            empty_coordinates.push_back({i,j});
        }
    }
    ...
    vector<pair<int,int>> way = generate_adv_path(0,0,0,10,x,y,table,y-1,x-1);
    int sum =0;
    for(pair<int,int> step : way){
    ...
    for(pair<int,int> i : empty_coordinates){
